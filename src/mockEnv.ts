...
console.log('mockEnv.ts: Starting execution');
console.log('mockEnv.ts: VITE_DEV_INIT_DATA =', import.meta.env.VITE_DEV_INIT_DATA);

if (!window.Telegram?.WebApp) {
   console.log('mockEnv.ts: Creating mock WebApp');
   // Build a minimal mock that satisfies the app's usage of the WebApp API
   const noop = () => {};
   const noopOff = () => {};

   const mockWebApp = {
     initData: import.meta.env.VITE_DEV_INIT_DATA || generateMockInitData(),
     initDataUnsafe: {
       user: {
         id: 0,
         first_name: "Test",
